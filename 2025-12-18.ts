...
/**
 * 1. 배열의 첫 번째 요소를 반환하는 함수를 작성하세요. 배열의 요소 타입에 관계없이 작동해야 합니다.
 *
 * 함수 이름: getFirstElement
 * 입력: 제네릭 배열
 * 출력: 배열의 첫 번째 요소
 */
// 매개변수, 리턴타입 정의 필요
...
/**
 * 2. 숫자 배열인지 문자열 배열인지 확인하는 함수를 작성하세요.
 *
 * 함수 이름: isNumberArray
 * 입력: 제네릭 배열
 * 출력: 배열이 숫자 배열이면 true, 그렇지 않으면 false
 */
// 매개변수, 리턴타입 정의 필요
 ...
 */
// 조건부 타입 정의
type IsArray<T> = T extends Array<any> ? true : false /* 여기에 작성 */;

/**
 * 4. 객체의 모든 속성에 대해 기본값을 추가하는 타입을 작성하세요.
 *
 * 유틸리티 타입 정의:
 * - 타입 이름: WithDefault<T>
 * - 입력: 객체 타입 T
...
/**
 * 5. 사용자 정보를 나타내는 객체 배열에서 특정 속성만 추출하는 함수를 작성하세요.
 *
 * 함수 이름: pluck
 * 입력:
 * - 객체 배열: 제네릭 타입 배열
...
/**
 * 1. 웹 애플리케이션에서 사용할 버튼의 스타일을 선택하는 함수를 작성하세요.
 *
 * 리터럴 타입 정의:
 ...
 */
function getButtonClass(style: 'primary' | 'secondary' | 'danger' /* 여기에 작성 */): string {
  // 여기에 구현
  switch (style) {
    case 'primary':
      return 'btn-primary';
    ...
    case 'danger':
      return 'btn-danger';
    default:
      throw new Error('올바른 버튼 스타일이 아닙니다.');
  }
}
...
/**
 * 2. 서버에서 데이터를 요청할 때 발생하는 상태를 처리하는 함수를 작성하세요.
 *
 * 리터럴 타입 정의:
 ...
 * - 출력: 상태에 따라 메시지를 반환
 */
function handleRequestState(state: 'loading' | 'success' | 'error' /* 여기에 작성 */): string {
  // 여기에 구현
  switch (state) {
    case 'loading':
    ...
    default:
      throw new Error('올바른 요청 상태가 아닙니다.');
  }
}
