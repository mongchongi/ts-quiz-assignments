 ...
 */
// 변수 선언과 초기값 지정
let productName: string = '아식스 젤 카야노 14'; // 상품 이름
let productPrice: number = 179000; // 상품 가격
 ...
 */
function stringifyValue(value: string | undefined | null): string {
  // 여기에 구현
  if (!value) {
    return '값이 없습니다';
  }
...
console.log(compareValues(false, 0)); // 느슨한 동등
...
function getProductNamesAndPrices(products1: [string, number, boolean][]): [string, number][] {
  // 여기에 구현
  return products1.map(function ([name, price]) {
    return [name, price];
  });
}

// 2. 재고가 있는 상품만 반환,리턴타입 정의필요
function getAvailableProducts(products1: [string, number, boolean][]): [string, number, boolean][] {
  // 여기에 구현
  return products1.filter(function ([name, price, isStock]) {
    return isStock;
  });
}
...
function getProductsByCategory(category: string): string[] {
  // 여기에 구현
  return products2.reduce(function (result: string[], product) {
